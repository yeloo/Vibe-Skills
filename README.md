...
- [`references/index.md`](references/index.md)：contracts / registries / matrices / ledgers / overlay packs 导航入口。
- [`scripts/README.md`](scripts/README.md)：governance / verify / common / router / overlay surfaces 总入口。
- [`docs/plans/2026-03-08-repo-full-cleanup-master-plan.md`](docs/plans/2026-03-08-repo-full-cleanup-master-plan.md)：当前 cleanup-first umbrella plan。
- [`third_party/README.md`](third_party/README.md)：canonical repo 的第三方合规边界说明。

## Repo Planes

| Plane | Paths | Role |
| --- | --- | --- |
| Canonical source | root `config/`, `docs/`, `protocols/`, `references/`, `scripts/`, root entry files | 唯一真源；所有治理修改先落这里 |
| Bundled mirrors | `bundled/skills/vibe/**`, `bundled/skills/vibe/bundled/skills/vibe/**` | packaged / compatibility mirrors；只能由 sync 生成 |
| Runtime artifacts | `outputs/**`, `.tmp/`, `.serena/` | 运行态证据、缓存与 scratch；不应伪装成长期资产 |
| Compliance boundary | `third_party/**`, `THIRD_PARTY_LICENSES.md`, `NOTICE` | license / notice / attribution surface |
## 生态组成
...
| `docs/ecosystem-absorption-dedup-governance.md` | VCO 生态吸收、去冗余与分层治理总纲（统一准入、落点、推广、回退规则） |
...
当前仓库已经进入 Wave19-30 的“全谱系吸收治理”阶段，新增重点包括：

- `mem0` 作为 **opt-in external preference backend**，不拥有 canonical memory 权。
- `Letta` 作为 **policy / contract source**，不拥有 runtime takeover 权。
- `Prompt-Engineering-Guide` 作为 **prompt pattern cards + risk checklist**，不形成第二 prompt surface。
- `browser-use` 作为 **BrowserOps provider candidate**，不替代现有 VCO/TuriX/Playwright/API 面。
- `Agent-S` 作为 **DesktopOps shadow/advisory contract source**，不成为默认执行面。
- `promotion-board + pilot scenarios + bundled sync + release cut` 作为 Wave27-30 的收口治理层。

关键文档入口：
- `docs/plans/2026-03-07-vco-full-spectrum-integration-plan.md`
...
### 17. Wave19-30 Full-Spectrum Integration（memory / prompt / browser / desktop 治理闭环）

Wave19-30 进一步把高价值外部项目吸收为 VCO 受治理的 plane / contract / provider 资产，而不是平行框架：

- **Admission / Dedup**：`docs/ecosystem-absorption-dedup-governance.md`、`docs/absorption-admission-matrix.md`
- **Memory Runtime v2**：`docs/memory-runtime-v2-integration.md`、`docs/mem0-optin-backend-integration.md`、`docs/letta-policy-integration.md`
- **Prompt Intelligence**：`docs/prompt-intelligence-governance.md`、`references/prompt-pattern-cards.md`、`references/prompt-risk-checklist.md`
- **BrowserOps Provider Plane**：`docs/browserops-provider-integration.md`、`references/browser-task-contract.md`
- **DesktopOps Shadow Plane**：`docs/agent-s-shadow-integration.md`、`references/aci-dsl.md`、`references/openworld-task-contract.md`
- **Conflict / Promotion / Eval**：`docs/cross-plane-conflict-governance.md`、`docs/promotion-board-governance.md`、`docs/pilot-scenarios-and-eval.md`

关键原则：
- 单一控制面仍是 VCO
- 不引入第二默认执行 owner
- 不引入第二 canonical memory truth-source
- 所有新增吸收面默认从 `off/shadow` 起步，并受 verify gates 与 rollback 规则约束
