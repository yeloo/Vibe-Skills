param(
    ...
    function Write-GateArtifacts {
        ...
        $jsonPath = Join-Path $dir 'vibe-cross-plane-replay-gate.json'
        $mdPath = Join-Path $dir 'vibe-cross-plane-replay-gate.md'
        Write-VgoUtf8NoBomText -Path $jsonPath -Content ($Artifact | ConvertTo-Json -Depth 100)

        $lines = @(
            '# VCO Cross-Plane Replay Gate',
            '',
    ...
    [pscustomobject]@{ path = 'references/cross-plane-replay-ledger.md'; keywords = @(
    'plane_id',
    'evidence_refs',
    'rollback_token'
) }
    ...
    $artifact = [pscustomobject]@{
        gate = 'vibe-cross-plane-replay-gate'
        repo_root = $repoRoot
