param(
    ...
    function Write-GateArtifacts {
        ...
        $jsonPath = Join-Path $dir 'vibe-desktopops-softrollout-gate.json'
        $mdPath = Join-Path $dir 'vibe-desktopops-softrollout-gate.md'
        Write-VgoUtf8NoBomText -Path $jsonPath -Content ($Artifact | ConvertTo-Json -Depth 100)

        $lines = @(
            '# VCO DesktopOps Soft Rollout Gate',
            '',
    ...
    $artifact = [pscustomobject]@{
        gate = 'vibe-desktopops-softrollout-gate'
        repo_root = $repoRoot
